...
class BelizeXSDK {
  ...
  async getPairAddress(tokenA, tokenB) {
    ...
    const { output } = await this.factory.query.getPairAddress(
      this.factoryAddress,
      { gasLimit },
      tokenA,
      tokenB
  ...
  async getAllPairsLength() {
    ...
    const { output } = await this.factory.query.allPairsLength(this.factoryAddress, { gasLimit });
  ...
  async getPairByIndex(index) {
    ...
    const { output } = await this.factory.query.getPairByIndex(
      this.factoryAddress,
      { gasLimit },
      index
    );
  ...
  async getReserves(pairAddress) {
    ...
    const { output } = await pair.query.getReserves(pairAddress, { gasLimit });
  ...
  async getTotalSupply(pairAddress) {
    ...
    const { output } = await pair.query.totalSupply(pairAddress, { gasLimit });
  ...
  async getBalanceOf(pairAddress, account) {
    ...
    const { output } = await pair.query.balanceOf(pairAddress, { gasLimit }, account);
  ...
  async getAmountOut(pairAddress, amountIn, reserveIn, reserveOut) {
    ...
    const { output } = await pair.query.getAmountOut(
      pairAddress,
      { gasLimit },
      amountIn,
      reserveIn,
  ...
  async swapExactTokensForTokens(params, signer) {
    ...
    const { result, output } = await this.router.tx
      .swapExactTokensForTokens({ gasLimit }, amountIn, amountOutMin, path, to, deadline)
      .signAndSend(signer);
  ...
  async swapTokensForExactTokens(params, signer) {
    ...
    const { result, output } = await this.router.tx
      .swapTokensForExactTokens({ gasLimit }, amountOut, amountInMax, path, to, deadline)
      .signAndSend(signer);
  ...
  async getAmountsOut(amountIn, path) {
    ...
    const { output } = await this.router.query.getAmountsOut(
      this.routerAddress,
      { gasLimit },
      amountIn,
      path
  ...
  async getAmountsIn(amountOut, path) {
    ...
    const { output } = await this.router.query.getAmountsIn(
      this.routerAddress,
      { gasLimit },
      amountOut,
      path
