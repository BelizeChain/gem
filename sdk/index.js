...
class GemSDK {
     ...
     * @param {number} gasLimit - Gas limit (default: 100000000000)
     * @returns {Promise<Object>} Transaction result
     */
    async dallaTransfer(contractAddress, signer, to, amount, gasLimit = 100000000000) {
    ...
    async nftMint(contractAddress, signer, to, uri) {
        ...
        const tx = await contract.tx.mint(
            { gasLimit: gasRequired },
            to,
            uri
        );
    ...
    async daoCreateProposal(contractAddress, signer, description) {
        ...
        const tx = await contract.tx.createProposal(
            { gasLimit: gasRequired },
            description
        );
    ...
    async daoVote(contractAddress, signer, proposalId, support) {
        ...
        const tx = await contract.tx.vote(
            { gasLimit: gasRequired },
            proposalId,
            support
        );
    ...
    async faucetClaim(contractAddress, signer) {
        ...
        const tx = await contract.tx.claim(
            { gasLimit: gasRequired }
        );
